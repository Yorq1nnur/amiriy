...
class BannerRepository {
  ...
  Stream<List<BannerModel>> getBannerStream() async* {
    try {
      // Listen to changes in the "banners" collection
      await for (var snapshot in FirebaseFirestore.instance
          .collection(AppConstants.banners)
          .snapshots()) {
        // Convert the snapshot documents to BannerModel objects
        List<BannerModel> banners =
            snapshot.docs.map((e) => BannerModel.fromJson(e.data())).toList();

        // Yield the list of banners
        yield banners;
      }
    } catch (e) {
      // Handle any errors
      UtilityFunctions.methodPrint(
        "Error getting banners: $e",
      );
    }
  }
}
